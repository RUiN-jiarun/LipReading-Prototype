...
class LRWDataset(Dataset):
    def __init__(self, phase, args):
        ...
        for (i, label) in enumerate(self.labels):
            ...
            files = glob.glob(os.path.join('lrw_roi_80_116_175_211_npy_gray_pkl_jpeg', '*.pkl'))
            files = sorted(files)
    ...
    def __getitem__(self, idx):
        ...
        inputs = [cv2.imdecode(img, cv2.IMREAD_COLOR) for img in inputs]
        print("inputs")
...
if (__name__ == '__main__'):
    ...
    dataset=LRWDataset('test','')

    print(len(dataset))
    video=dataset[0].get('video')
    for frame in video:
        print(frame)
