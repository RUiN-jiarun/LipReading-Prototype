...
# jpeg = TurboJPEG(r'C:\libjpeg-turbo-gcc64\bin\libjpeg-62.dll')
...
class LRWDataset(Dataset):
    ...
    def __init__(self,route):
        ...
        files = glob.glob(os.path.join(route,'*.mp4'))
        # files = [route]
        for file in files:
            savefile = file.replace(route, target_dir).replace('.mp4', '.pkl')
            savepath = os.path.split(savefile)[0]
            if (not os.path.exists(savepath)):
                os.makedirs(savepath)
        ...
        self.list += [(file, 0) for file in files]
    ...
    def __getitem__(self, idx):
        ...
        savename = self.list[idx][0].replace(self.route, target_dir).replace('.mp4', '.pkl')
...
def prepareData(route):
    loader = DataLoader(LRWDataset(route),
                        batch_size=96,
                        num_workers=0,
                        shuffle=False,
                        drop_last=False)
