...
from scripts import prepare_lrw
from detect_open_mouth import preprocess
...
parser.add_argument('--test', type=str2bool, required=False,default=True)

# load opts
parser.add_argument('--weights', type=str, required=False, default='checkpoints/lrw-cosine-lr-acc-0.85080.pt')

# save prefix
parser.add_argument('--save_prefix', type=str, required=False,default='checkpoints/lrw-baseline/')
...
def LipRead(path):
    prepare_lrw.prepareData(path)
    with torch.no_grad():
        dataset = Dataset('test', args)
        print('Start Testing, Data Length:', len(dataset))
        loader = dataset2dataloader(dataset, 1, args.num_workers, shuffle=False)
        ...
        for (i_iter, input) in enumerate(loader):
            ...
            print(m2)
            print(m3)
            return [labels[m1],labels[m2],labels[m3]]


if(__name__ == '__main__'):
    # preprocess()
    # os.remove('lrw_roi_80_116_175_211_npy_gray_pkl_jpeg/output.pkl')
    res= LipRead("data/output.mp4")
    print(res)
    exit()
    # train()
